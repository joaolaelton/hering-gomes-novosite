  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        {statCards.map(({ icon: Icon, label, value, path, color }) => (
          <div key={label} className="bg-charcoal-light border border-white/5 rounded-xl p-6 shadow-lg">
            ...
            {path ? (
              <Link href={path} className="mt-4 flex items-center gap-1 text-xs text-white/40 hover:text-gold transition-colors">
                Gerenciar <ArrowRight size={12} />
              </Link>
            ) : (
              <div className="mt-4 flex items-center gap-1 text-xs text-transparent">
                <span className="opacity-0">Gerenciar</span>
              </div>
            )}
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Link href="/admin/blog" className="bg-charcoal-light hover:bg-white/5 border border-white/10 rounded-xl p-6 text-left transition-colors flex items-center justify-between group">
          <div>
        ...
        <Link href="/admin/reviews" className="bg-charcoal-light hover:bg-white/5 border border-white/10 rounded-xl p-6 text-left transition-colors flex items-center justify-between group">
          <div>
